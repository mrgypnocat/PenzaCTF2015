...
class Action():
    def __init__(self, _host, _port):
        self.url_base = "http://{0}:{1}".format(str(_host), str(_port))
    ...
    def create_user(self, name, password):
        ...
        if self.status == 200:
            _result = dict(response.request._cookies)
            try:
                logging.info('new user %s:%s on host %s was created, cookies = %s', name, password, url, _result)
            except:
                pass
            return _result
    ...
    def add_flag(self, cookies, _flag, title, text):
        append = "/add"
        return self.edit_flag(cookies, _flag, title, text, append)

    def edit_flag(self, cookies, _flag, title, text, apend_url):
        ...
        if self.status == 200:
            try:
                logging.info('putted flag %s to host %s: %s', _flag, url, self.status)
            except:
                pass
            return self.status

        try:
            logging.error('failed to put flag %s to host %s: %s', _flag, url, self.status)
        except:
            pass
        return None
    ...
    def check_flag(self, cookies, flag):
        ...
        try:
            logging.error('flag %s on host %s is not found', flag, url)
        except:
            pass
        return None
    ...
    def get_flag(self, cookies):
        ...
        try:
            ...
            self.status = response.status_code
            tmpres = re.findall(r'<span>[^<]+</span>', response.text)[0]
            return tmpres.replace('<span>','').replace('</span>','').replace('\n','')
        except:
            pass

        try:
            logging.error('flag %s on host %s was not founded', self.result, url)
        except:
            pass
...
def putFlag(host, port, flag):
    ...
    if not action.knock_Knock("/", cookies_register):
        try:
            logging.error('can not knock to %s', str(action.url_base)+"/")
        except:
            pass
...
def getFlag(host, port, login, password):
    action = Action(str(host), str(port))

    state = action.login_chk(login, password)
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        myaction = Action("127.0.0.1", "8000")
        #print myaction.create_user("bot", "botpass")
        mycookies = myaction.login_chk("bot", "botpass")
        #print myaction.add_flag(mycookies, "BotTestFlag", "title", "text")
        print myaction.check_flag(mycookies, "BotTestFlag")
        print myaction.get_flag(mycookies)
        print myaction.getMyPostId(mycookies)
    ...
    try:
        ...
        if "put" == cmd:
            if len(sys.argv) < 5:
                exit(STATUS_CHECKER_ERROR)
            flag = sys.argv[4]
            print str(putFlag(host, port, flag)).replace(' ','')
            ...
            try:
                state = sys.argv[4]
                login = str(state).split('.')[0]
                password = str(state).split('.')[1]
                print getFlag(host, port, login, password)
            except:
                exit(STATUS_CHECKER_ERROR)
