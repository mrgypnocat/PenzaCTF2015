...
def SConnect(serverIP):
    ...
    while (1):
        ...
        if not key_present:
            ...
            # does it work? do not touch!
            reply = str(reply).replace("b'", '').replace("'", "") \
                .replace("\\\\n", "\n").replace('"', "").split(' ')

            pubkey_str = reply[7] + ' ' + reply[8] + ' ' + reply[9] + ' ' + \
                reply[10] + ' ' + reply[11]
            privkey_str = reply[15] + ' ' + reply[16] + ' ' + reply[17] + ' ' \
                + reply[18] + ' ' + reply[19] + ' ' + reply[20] + ' ' + reply[21]
...
def CListen(server_socket, key):
    ...
    Csock = socket()
    Csock.bind((ADDRESS, CPORT))  # адрес и порт клиента
    ...
    try:
        ...
        reply = str(other_data).replace("b'", '').replace("'", "").\
            replace("\\n", "\n").replace('"', "").split("\n")
        ...
        while True:
            ...
            if str1 == "quit":
                return
            conn.sendall(str1.encode('utf-8'))
...
def CConnect(name, key, client_name, client_info):
    ...
    try:
        Csock.connect((client_info.IP, CPORT))
        print ('Connected')
        ...
        if (msg1.command != 'auth_done' or
            Other_key.decrypt(msg1.signature) != h):
            print('Verification failed')
            return
        ...
        while True:
            ...
            if (str2 == "quit"):
                return
            Csock.sendall(str2.encode('utf-8'))
